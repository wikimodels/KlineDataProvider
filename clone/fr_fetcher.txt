...
try:
    ...
    async def get_coins(): return None 
...
async def run_fr_update_process():
    ...
    try:
        ...
        if fr_data is None or not fr_data:
            logger.warning("[CRON_JOB] 2/3: Сбор FR вернул пустой словарь или None. Обновление FR отменено.")
            return
        ...
        if not redis_conn:
            logger.critical("[CRON_JOB] Не удалось подключиться к Redis. Сохранение FR отменено.")
            return
            
        # --- ИСПРАВЛЕНИЕ: Оборачиваем fr_data в словарь с ключом 'data' ---
        # Это устраняет TypeError, так как save_to_cache ожидает словарь с ключом 'data'
        data_to_save = {
            "data": fr_data, 
            "timeframe": "global_fr"
        }
