...
async def fetch_market_data(
    ...
    tasks_to_run = task_builder.prepare_tasks( # <-- ИСПРАВЛЕНИЕ: prepare_fetch_tasks -> prepare_tasks
        coins, 
        timeframe, 
        prefetched_fr_data
    )
    ...
    klines_oi_tasks = [t for t in tasks_to_run if t['task_info']['data_type'] in ['kline', 'oi']]
    ...
    # Собираем все задачи
    for task in klines_oi_tasks:
        strategy_func = task["fetch_strategy"]
        task_info = task["task_info"]
        async_tasks.append(strategy_func(aiohttp.ClientSession(), task_info, semaphore))
