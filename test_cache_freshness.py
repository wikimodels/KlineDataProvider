...
from typing import List, Dict, Any, Optional # <-- НОВЫЙ ИМПОРТ
...
# ============================================================================
# === (НОВЫЙ БЛОК) ХЕЛПЕРЫ ДЛЯ УГЛУБЛЕННОЙ ПРОВЕРКИ 8H ===
# ============================================================================
...
async def _verify_8h_aggregation_logic(client: httpx.AsyncClient) -> bool:
    ...
    try:
        ...
        resp_4h = await client.get("/cache/4h")
        resp_4h.raise_for_status()
        cache_4h = resp_4h.json()
        
        resp_8h = await client.get("/cache/8h")
        ...
        if not coins_to_check:
            ...
            return True # Не провал, но и не успех
        ...
        for coin_8h_data in coins_to_check:
            ...
            for candle_8h in candles_8h[-10:]:
                ...
                # 8h свеча (openTime T) состоит из:
                # 4h-свеча-№1 (openTime T)
                # 4h-свеча-№2 (openTime T + 4h_ms)
...
async def run_freshness_test():
    ...
    async with httpx.AsyncClient(base_url=BASE_URL, timeout=120.0) as client:
        ...
        for key in CACHE_KEYS_TO_TEST:
            ...
            try:
                ...
                if not last_close_time_ms:
                    # (Проверяем, пустой ли 'data'. Если да, то это нормально)
                    if not data.get("data"):
                         log.warning(f"       ⚠️  [ПРЕДУПРЕЖДЕНИЕ] 'cache:{key}' пуст (нет 'data'), поэтому 'closeTime' отсутствует. (Аудит: {data.get('audit_report')})")
                ...
                # --- (НОВЫЙ БЛОК) Шаг 2Б: Проверка Глубины (Количества свечей) ---
                ...
                for coin_data in coins_data_list[:10]:
                    ...
                    # Ожидаем 399 (т.к. data_processing отрезает до 399)
                    if candle_count > 399:
        ...
        # --- (НОВЫЙ БЛОК) Шаг 3: Проверка логики 8h ---
